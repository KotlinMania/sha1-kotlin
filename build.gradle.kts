...
kotlin {
    ...
    val xcf = XCFramework("Sha1")

    macosArm64 {
        binaries.framework { baseName = "Sha1"; xcf.add(this) }
    }
    iosArm64 {
        binaries.framework { baseName = "Sha1"; xcf.add(this) }
    }
    ...
    swiftExport {
        moduleName = "Sha1"
        flattenPackage = "io.github.kotlinmania.sha1"
    }

    android {
        namespace = "io.github.kotlinmania.sha1"
        compileSdk = 34
        minSdk = 24
...
mavenPublishing {
    ...
    coordinates(group.toString(), "sha1-kotlin", version.toString())

    pom {
        name.set("sha1-kotlin")
        description.set("Kotlin Multiplatform port of RustCrypto/hashes - SHA-1 hash function")
        inceptionYear.set("2026")
        url.set("https://github.com/KotlinMania/sha1-kotlin")
        ...
        scm {
            url.set("https://github.com/KotlinMania/sha1-kotlin")
            connection.set("scm:git:git://github.com/KotlinMania/sha1-kotlin.git")
            developerConnection.set("scm:git:ssh://github.com/KotlinMania/sha1-kotlin.git")
        }
    }
}
...
// Make `build` compile every configured target instead of only the cheap
// host-portable subset. The stock Gradle `build` graph wires
// `*MainClasses` and `*TestClasses` along with host-runnable test links,
// but skips XCFramework assembly and device/Linux-arm64/androidNative test
// binary linking. The explicit set below is the audit contract for this
// repo's current target surface; the late afterEvaluate matcher is a
