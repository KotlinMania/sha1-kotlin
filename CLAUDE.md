# Claude Code Project Instructions — sha1-kotlin

## Project Overview

This is **sha1-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`sha1`](https://crates.io/crates/sha1).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/sha1/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/sha1>
