...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Sha1.swiftmodule/`
...
//      `import Sha1` above would fail at compile time.
//
//   2. The static archive `libSha1.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Sha1" }` and
//      `flattenPackage = "io.github.kotlinmania.sha1"` configuration in
...
final class Sha1ExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Sha1 swift module imported cleanly")
    }
}
